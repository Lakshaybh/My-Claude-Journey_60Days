...
**Rule:** In plain prose, remove emojis entirely and rewrite the line without them. In UI or React contexts (JSX/TSX, component code, design specs that render to components), do not use emoji glyphs at all; use [lucide-react](https://lucide.dev) icon components only (for example `<Rocket />`, `<Lightbulb />`, `<CheckCircle />`). Import them from `lucide-react` and size/color them through props rather than dropping an emoji character into the markup.
...
>
> <li><Rocket size={16} /> Launch Phase</li>
> <li><Lightbulb size={16} /> Key Insight</li>
> <li><CheckCircle size={16} /> Next Steps</li>
> ```
