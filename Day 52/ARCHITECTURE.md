...
| Hosting | **Render (Free Web Service tier)** — ✅ **LIVE Day 56:** https://querymind-3msv.onrender.com | Free, deploys directly from a GitHub repo (Root Directory set to `Day 52/querymind`), supports environment variables for the API key, and runs Python/Uvicorn natively. Deployed early (Day 56) rather than Day 9 as originally scheduled — a deliberate scope compression to have a working shareable demo mid-capstone; see the Blueprint Day 6 note for why. |
| Other libraries | `groq` (official Python SDK), `python-dotenv` (local env vars), `pydantic` (bundled with FastAPI, request/response validation) | All free, minimal, and match the "no new stack to learn" constraint. |
